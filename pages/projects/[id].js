      ...
      <section className="w-full h-full font-sans bg-[hsl(0,0%,97%)] p-3 py-20 md:px-7 grid items-center justify-center">
        <div className="py-16">
          <div className="justify-self-center text-center">
            <h4 className="font-semibold text-xl md:text-2xl text-color-h">{title[0]}</h4>
          </div>
        </div>
              ...
              pro.gallery.map((img) => (
                <div key={img.id} className="relative w-[330px] h-[450px] cursor-pointer overflow-hidden group duration-500">
                  <div className="w-full h-full overflow-hidden">
                    <img src={img.image} alt={img.catergory} className="w-full h-full duration-500 group-hover:scale-110 object-cover" />
                  </div>
                </div>
              ))
            ))}
